...
def graph_search(problem, fringe):
    ...
    ## Lista abierta, cogemos el nodo de donde partimos
    fringe.append(Node(problem.initial))

    # Mientras existan nodos en la lista abierta
    while fringe:
        node = fringe.pop()     ## Sacamos uno

        ## Es el final/objetivo?
        ...
        ## Ya hemos pasado por él?
        if node.state not in closed:

            ## Lista cerrada  metemos el nodo al no haber pasado por ahí
            closed[node.state] = True

            ## Expandimos la lista abierta
...
def branchAndBound(problem):
    return graph_search(problem, ramificacionAcotacion(problem))

## Parte 2 - Ramificación y Acotación con Subestimación
def branchAndBoundSubestimation(problem):
    return graph_search(problem, ramificacionAcotacionSubestimacion(problem))
